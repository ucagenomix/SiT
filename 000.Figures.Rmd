...
# Supplementary dotplot (mob+hip79)
...
DefaultAssay(object = hip142079) <- "ISO"
DotPlot(object = hip142079, features = lst, scale=TRUE, scale.max = 100, scale.min = 0) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
dev.off()

tiff("figures/dotplot.hip.gene.tif", units="in", width=24, height=6, res=300)
DefaultAssay(object = hip142079) <- "SCT"
print(DotPlot(object = hip142079, features = general, scale=TRUE, scale.max = 100, scale.min = 0, cols = c("lightgrey", "red")) + theme(axis.text.x = element_text(angle = 90, hjust = 1))) # 
dev.off()
...
tiff("figures/fig2e-1.tif", units="in", width=8, height=6, res=300)
ggplot(data=hip142079@meta.data, aes(x=ClusterName,y=AtoI_ratio, group=ClusterName))+
  geom_boxplot(outlier.shape = NA, fill=col, color="black") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_y_continuous(limits=c(0,0.4), breaks=seq(0,0.4,0.1), expand = c(0, 0))
dev.off()
...
#SpatialFeaturePlot(hip142079, features = c("AtoI_detected"), pt.size.factor=1.6, alpha = c(1, 1), max.cutoff = c(30))
...
tiff("figures/fig2e-2.tif", units="in", width=8, height=6, res=300)
pheatmap(avg@assays$SCT@scale.data[c("Adar","Adarb1","Adarb2"),], cluster_rows=FALSE, cluster_cols=FALSE, fontsize = 8, clustering_method = "complete", clustering_distance_rows = "manhattan")
dev.off()
...
# Main fig.2e (hip, Snap25 isoforms)
...
# Supplementary single cell deconvolution per region (mob, Plp1 story)
...
tiff("figures/sup.mob.plp1.tif", units="in", width=18, height=4, res=300)
plot_grid(p1,p2,ncol=5)
dev.off()

tiff("figures/sup.mob.plp1-2.tif", units="in", width=10, height=4, res=300)
plot_grid(p3,p4,ncol=2)
dev.off()

#sub <- subset(tepe, subset = ClusterName %in% c("MyOligo","OEC1","OEC2","OEC3","OEC4","OEC5"))
#dim(sub)
#table(sub@active.ident)
#sub <- SetIdent(sub, value="ClusterName")
#markers <- FindMarkers(sub, ident.1="MyOligo", ident.2=c("OEC1","OEC2","OEC3","OEC4","OEC5"), only.pos = FALSE)
#head(markers,20)

```

# Supplementary spatial markers

```{r message=FALSE, warning=FALSE, echo=FALSE, eval=T, fig.height=4, fig.width=6}

DefaultAssay(hip142079) <- "SCT"
SpatialDimPlot(hip142079, pt.size.factor =1.6)

tiff("figures/sup.markers.hip.tif", units="in", width=14, height=12, res=300)
SpatialFeaturePlot(hip142079, features = c("Camk2n1","Myl4","Mal","Spink8","Mbp","Nnat","Slc18a2","Zic1"), slot="data", pt.size.factor=1.6, alpha = c(1, 1))
dev.off()

DefaultAssay(mob164315) <- "SCT"
SpatialDimPlot(mob164315, pt.size.factor =1.6)
...
# Main fig.2 - 86 editing sites heatmap
...
#ggplot(data=hip142079@meta.data, aes(x=ClusterName,y=geneRatio, group=ClusterName))+
#  geom_violin(draw_quantiles = c(.25, .5, .75, .95), na.rm = TRUE) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  #geom_text(aes(label=geneCount), vjust=1.5, color="black", position = position_dodge(0.9), size=3)+
#  scale_y_continuous(limits=c(0,0.4), breaks=seq(0,0.4,0.1), expand = c(0, 0)) +
#  coord_flip() + labs(title = "\n\nEditing ratio per region\n")
