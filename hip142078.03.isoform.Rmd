---
title: "Visium Spatial Tx - Mouse Hippocampus (hip142078) - 03.isoforms"
...
DefaultAssay(object = hip142078) <- "ISO"
total <- data.frame()
totaladj <- data.frame()
for (i in (1:12)){
  k <- i+1
  for (j in (k:13)){
    if(i != j){
      ...
      markers <- FindMarkers(object = hip142078, ident.1=clusters[i], ident.2=clusters[j])
...
write.table(totaladj, file="output/hip142078.isoswitch.top.csv", sep=",")
...
write.table(total, file="output/hip142078.isoswitch.csv", sep=",")
...
DefaultAssay(object = hip142078) <- "JUNC"
total <- data.frame()
totaladj <- data.frame()
for (i in (1:12)){
  k <- i+1
...
length(unique(totaladj$geneId))
print(unique(totaladj$geneId))
write.table(totaladj, file="output/hip142078.juncswitch.top.csv", sep=",")
...
length(unique(total$geneId))
print(unique(total$geneId))
write.table(total, file="output/hip142078.juncswitch.csv", sep=",")
