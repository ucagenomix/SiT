...
all = read.delim("nanopore/CBS2.snp5817.illumina_snpmatrix.csv", sep=",", stringsAsFactors = F)
data = all[,4:ncol(all)]
dim(data)
...
# Visualize editing sites expression colored by distance to 3p end

```{r message=FALSE, warning=FALSE, echo=TRUE, eval=T, fig.height=4, fig.width=6}

all = read.delim("dist3p.csv", sep=",", stringsAsFactors = F, header=F)
...
pdf("ggplot.editing.2.pdf", width=8, height=6, useDingbats=FALSE)
...
#  geom_text_repel(data=subset(df, df$illu.total > 100 & df$nano.total < 10), aes(label=Row.names), size=3) 
...
x <- unique(rownames(CBS1@assays$AtoI@counts)[grep(paste0("^",gene), rownames(CBS1@assays$AtoI@counts))])
a <- aggregate(CBS1@assays$AtoI@counts[x,], list(CBS1@meta.data$ClusterName), sum)
colnames(a) <- c("group1","A")
g <- aggregate(CBS1@assays$AtoI@data[x,], list(CBS1@meta.data$ClusterName), sum)
colnames(g) <- c("group2","G")
all <- cbind(a,g)
all$ratio <- 100* all$G / (all$A+all$G)
all[is.na(all)] <- 0
CBS1@meta.data$Gria2QR <- 0
all <- all[-c(13),]
for(i in 1:dim(all)[1]){
  CBS1@meta.data[which(CBS1@meta.data$ClusterName == clusters[i]),]$Gria2QR <- all[i,"ratio"]/100
}
SpatialFeaturePlot(CBS1, "Gria2QR")+ ggplot2::scale_fill_continuous(limits = c(0.0,1.0), breaks = c(0.0, 0.25, 0.50, 0.75, 1.0), type = "viridis")
...
all$ratio <- 100* all$G / (all$A+all$G)
all[is.na(all)] <- 0
CBS2@meta.data$test <- 0
for(i in 1:dim(all)[1]){
  CBS2@meta.data[which(CBS2@meta.data$ClusterName == clusters[i]),]$test <- all[i,"ratio"]/100
}

p1 <- SpatialFeaturePlot(CBS2, "test")+ ggplot2::scale_fill_continuous(limits = c(0.0,0.25), type = "viridis")
p2 <- SpatialFeaturePlot(CBS2, gene, slot="scale.data")+ ggplot2::scale_fill_continuous(type = "viridis")
plot_grid(p1,p2)
...
CBS2@meta.data$Dnm1bxx <- 0
for(i in 1:dim(all)[1]){
  CBS2@meta.data[which(CBS2@meta.data$ClusterName == clusters[i]),]$Dnm1bxx <- all[i,"ratio"]/100
}
p8 <- SpatialFeaturePlot(CBS2, "Dnm1bxx")+ ggplot2::scale_fill_continuous(limits = c(0.0,1.0), breaks = c(0.0, 0.25, 0.50, 0.75, 1.0), type = "viridis")

#df[grep("^Ndufa12", df$V4),]

pdf("specific.pdf", width=20, height=12, useDingbats=FALSE)
...
CBS2@meta.data$EditingRatio <- 0
for(i in 1:dim(all)[1]){
  CBS2@meta.data[which(CBS2@meta.data$ClusterName == clusters[i]),]$EditingRatio <- all[i,"ratio"]/100
}
SpatialFeaturePlot(CBS2, "EditingRatio")+ ggplot2::scale_fill_continuous(limits = c(0.0,0.25), breaks = c(0.0, 0.25, 0.50, 0.75, 1.0), type = "viridis")
