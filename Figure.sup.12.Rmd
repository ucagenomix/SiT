...
#MOB
MOB[['AtoI_A']] <- Matrix::colSums(MOB@assays$AtoI@counts[,])
MOB[['AtoI_G']] <- Matrix::colSums(MOB@assays$AtoI@data[,])
MOB[['AtoI_total']] <- MOB[['AtoI_A']]+MOB[['AtoI_G']]
MOB[['AtoI_ratio']] <- MOB[['AtoI_G']]/MOB[['AtoI_total']]
exlude.MOB <- which(is.na(MOB[['AtoI_ratio']]))
...
# ---------------- MOB
df <- MOB@meta.data[-exlude.MOB, ]
df$ClusterName <- factor(df$ClusterName, levels=c("Glomerular Layer (GL)",
                                                  "Granule Cell Layer (GCL+RMS)",
                                                  "Mitral Cell Layer (MCL)",
                                                  "Olfactory Nerve Layer (ONL)",
                                                  "Outer plexiform Layer (EPL)"))

df %>% group_by(ClusterName) %>%
                    summarize(mean(AtoI_ratio)) 
#Boxplot
gg.box.global.mob <- ggplot(data=df, aes(x=ClusterName,y=AtoI_ratio, group=ClusterName, fill=ClusterName))+
  geom_boxplot(outlier.shape = NA) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_y_continuous(limits=c(0,0.4), breaks=seq(0,0.4,0.1), expand = c(0, 0)) +
  ylab("Editing ratio") + xlab("") + theme_bw() 
...
pdf("global.editing_CBS2.pdf", width = 12, height = 8)
...
#Fibertracts 
#Thalamus 
pdf("global.editing_CBS1.pdf", width = 12, height = 8)
gg.box.global / (gg.global[["Fiber tracts"]]+ggtitle("Fiber tract simulation") | gg.global[["Thalamus"]]+ggtitle("Thalamus simulation"))
dev.off()


# MOB
# "For region:  Mitral Cell Layer (MCL)"
#"pnorm of lower tail:  0.999999453732962"
#"pnorm of higher tail:  5.46267038084112e-07"
#"For region:  Olfactory Nerve Layer (ONL)"
#"pnorm of lower tail:  3.3948504946477e-18"
#"pnorm of higher tail:  1"
pdf("global.editing.MOB.pdf", width = 16, height = 10)

print(gg.box.global.mob / (gg.global.MOB[["Mitral Cell Layer (MCL)"]]+ggtitle("Mitral Cell Layer simulation") | gg.global.MOB[["Olfactory Nerve Layer (ONL)"]]+ggtitle("Olfactory Nerve Layer")))
dev.off()
