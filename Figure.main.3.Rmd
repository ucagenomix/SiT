...
pdf("figures/Figure.main.3a.pdf", width=12, height=8, useDingbats=FALSE)
plot_grid(p1,p2)
dev.off()
...
library("ggpubr")
p1 <- ggplot(df, aes(x=region, y=isoform/gene, fill=region)) + geom_boxplot(outlier.shape = NA)+ ggtitle("Isoforms/Genes per region CBS2") + theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + stat_compare_means(comparisons = list(c("Thalamus","CA3")),symnum.args = list(cutpoints = c(0, 0.0001, 0.001, 0.01, 0.05, 1), symbols = c("****", "***", "**", "*", "ns")))

p1 <- ggplot(df, aes(x=region, y=isoform/gene, fill=region)) + geom_boxplot(outlier.shape = NA)+ ggtitle("Isoforms/Genes per region CBS2") + theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + stat_compare_means(method="t.test", comparisons = list(c("Isocortex-1","Isocortex-2")))
...
p2 <- ggplot(df.cbs1, aes(x=region, y=isoform/gene, fill=region)) + geom_boxplot(outlier.shape = NA)+ ggtitle("Isoforms/Genes per region CBS1") + theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + stat_compare_means(method="t.test", comparisons = list(c("Isocortex-1","Isocortex-2")))

plot_grid(p2,p1)

pdf("Fig.3e.pdf", width=12, height=6, useDingbats=FALSE)
plot_grid(p2,p1)
dev.off()
